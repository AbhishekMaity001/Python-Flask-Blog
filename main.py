...
class Posts(db.Model):
    ...
    tagline = db.Column(db.String(50), unique=True,nullable=True)
...
@app.route('/')
def home():
    # 3 cases either user will be in 1st page or middle page or last page

    posts = Posts.query.filter_by().all()
    last = math.ceil(len(posts)/int(params['no_of_posts'])) #typecasting it coz json is giving us the string data
    #[0:params['no_of_posts']]  # all()
    ...
    #posts = Posts.query.filter_by()[0:params['no_of_posts']]   #all()
    ...
    if('user' in session and session['user'] == params['admin_user']):
        if(request.method == 'POST'):
            ...
            if sno == '0':
                ...
                post.date = date
                db.session.commit()
                return redirect('/edit/'+sno) #after redirectiong this request will no longer be the POST action so he will be get render_temp below
        post = Posts.query.filter_by(sno=sno).first()
...
def contact() :
    ...
    if(request.method == 'POST'):
        ...
        '''Add entry to the database (left sides variables are db variables!)'''
